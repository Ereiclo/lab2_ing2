...
app.get('/', async(req,res) => {
    ...
    if(author_name) {
        ...
        if((await songs_detailed_info.countDocuments(query)) === 0){
            ...
            const song_result = await songs_detailed_info.find(query,{release_date:1,duration_ms:1});
            const results = [];

            for await (song of song_result){
                console.log(song.duration);

                results.push({name:song.name,release_date:song.release_date,duration:toMinutesAndSeconds(song.duration_ms)});
            }

            res.json({result:{artist:author_name,songs:results},status:"Found"});
...
app.get('/song-detailed-info',async (req,res) => {
    ...
    if(song_name){
        // const query = {artists:{$regex:"Vladimir Horowitz"}};
        ...
        if((await songs_detailed_info.countDocuments(query)) === 0){
            ...
            const song_result = await songs_detailed_info.findOne(query,{release_date:1,duration_ms:1});
            // console.log(song_result)

            res.send({result:{name:song_name,release_date:song_result.release_date,duration:toMinutesAndSeconds(song_result.duration_ms)},status: "Found"});
        }
   }
    else {
